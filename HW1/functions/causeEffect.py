...
pos_patterns_first_cause = [
    ...
    '.*(منجر به|موجب|باعث|منجربه).*(است|هست|هستند|میشود|می شود|میباشد|می باشد|میباشند|می باشند|بود|بودند|باشد|باشند)',
...
def causeEffect(input: str):
    ...
    for i, pattern in enumerate(pos_patterns_first_cause):
        if re.search(pattern, input) is not None:
            input_pro = [part for part in re.sub(question_seperator[i], '*$*', input).split('*$*') if len(part) > 1]
    ...
    for i, pattern in enumerate(pos_patterns_second_cause):
        if pattern is not None and re.search(pattern, input) is not None:
            input_pro = [part for part in re.sub(question_seperator[i], '*$*', input).split('*$*') if len(part) > 1]
